...
import {
  requireCanSeeDocument,
  requireCanSeeRouting,
} from "./disseminationController";
import {
  DEFAULT_RENDER_PX,
  pdfPageSizes,
  renderPdfPage,
 ...
 */
export const routingSignSheet = async (
  req: FastifyRequest,
  res: FastifyReply,
) => {
  const params = req.query as { queueId?: string };
  if (!params.queueId) throw new ValidationError("INVALID REQUIRED ID");
  await requireCanSeeRouting(req, params.queueId);
  ...
  try {
    ...
    const mineIds = new Set(
      queue.signatotyArrangement
        .filter((a) => a.userId && a.userId === actorId)
        .map((a) => a.id),
    );
    const minePending = new Set(
      queue.signatotyArrangement
    ...
    for (const doc of queue.documents) {
      const boxesByPage = new Map<number, typeof doc.pages[number]["signCoor"]>();
      for (const p of doc.pages) boxesByPage.set(p.page, p.signCoor);

      let sizes: { page: number; widthPt: number; heightPt: number }[] = [];
      try {
      ...
      documents.push({
        id: doc.id,
        title: doc.title ?? doc.file?.fileName ?? "Document",
        pageCount: sizes.length,
        pages: sizes.map((s) => {
          const boxes = boxesByPage.get(s.page) ?? [];
          return {
            page: s.page,
            widthPt: s.widthPt,
            heightPt: s.heightPt,
            ...
            mine: boxes
              ...
              .map((b) => ({
                ...
                wBp: b.width,
                hBp: b.height,
                pending: minePending.has(b.signatoryArrangementId as string),
              })),
    ...
    const myAreas = documents.reduce(
      (n, d) =>
        n + d.pages.reduce((m, p) => m + p.mine.filter((b) => b.pending).length, 0),
      0,
    );

    return res.code(200).send({
      queueId: queue.id,
      title: queue.title ?? "Untitled routing",
      from: queue.fromRoom?.code ?? fullName(queue.user) ?? "—",
      sentAt: queue.timestamp,
      status: queue.status,
      signatories: queue.signatotyArrangement.map((a) => ({
        position: a.index + 1,
        name: fullName(a.user) ?? "Unassigned",
        position_title: a.user?.Position?.name ?? null,
        signed: a.status === 1,
        signedAt: a.signedAt,
 ...
 */
export const routingPageImage = async (
  req: FastifyRequest,
  res: FastifyReply,
) => {
  const params = req.query as { documentId?: string; page?: string; w?: string };
  if (!params.documentId) throw new ValidationError("INVALID REQUIRED ID");
  await requireCanSeeDocument(req, params.documentId);

  const page = Math.max(1, parseInt(params.page ?? "1", 10) || 1);
  const width = parseInt(params.w ?? "", 10) || DEFAULT_RENDER_PX;
  ...
  try {
    ...
    throw new NotFoundError("Page not available");
  }
};
