...
const canSeeRouting = async (
  ...
  const signs = await prisma.signatoryArrangement.count({
  ...
  return !!(await prisma.roomAuthorizedUser.findFirst({
 ...
 */
const requireOwnsRouting = async (req: FastifyRequest, queueId: string) => {
  const actorId = await callerUserId(req);
  if (!actorId) throw new UnauthorizedError("Not signed in");
  const queue = await prisma.signatureQueueRoom.findUnique({
    where: { id: queueId },
  ...
  if (!queue?.receivingRoomId) throw new NotFoundError("Not found");
  ...
  if (!member) {
    ...
    throw new UnauthorizedError("Only the sending office can change this.");
  }
  return actorId;
};
...
export const disseminationDocuments = async (
  ...
  // Only the people this routing actually involves.
  await requireCanSeeRouting(req, params.queueRoomId);
...
export const streamDocumentFile = async (
  ...
  await requireCanSeeDocument(req, params.id);
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (slots.length > 0) {
        ...
        if (arr.length === 0) {
          throw new ValidationError(
            "This routing has no signatories, so there is nothing to sign. " +
        ...
        if (unknown.length > 0) {
          throw new ValidationError(
            `Signature slot${unknown.length === 1 ? "" : "s"} ` +
              `${unknown.map((s) => `#${s}`).join(", ")} ` +
              `${unknown.length === 1 ? "has" : "have"} no signatory. ` +
              `This routing has ${arr.length} signator${
                arr.length === 1 ? "y" : "ies"
              } — use slot${arr.length === 1 ? " #1" : `s #1–#${arr.length}`}.`,
          );
        }
      }
      ...
      if (body.userId) {
        ...
        const recent = await tx.documentActivityLogs.findFirst({
          where: {
            userId: body.userId,
        ...
        if (recent) {
          await tx.documentActivityLogs.update({
            where: { id: recent.id },
...
export const uploadDisseminationDocument = async (
  ...
  try {
    ...
    if (!queueRoomId) throw new ValidationError("INVALID REQUIRED ID");
    await requireOwnsRouting(req, queueRoomId);
...
export const removeDisseminationDocument = async (
  ...
  await requireOwnsRouting(req, params.queueRoomId);
...
export const documentOverview = async (
  ...
  // else's totals.
  const ownLine = account?.User?.lineId ?? account?.lineId ?? null;
  if (!ownLine || ownLine !== params.lineId) {
    console.warn(
      `[overview] refused: account ${accountId} asked for line ${params.lineId}`,
    );
  ...
  try {
    ...
    const roomId: string | null = room?.id ?? null;
    ...
    const [
      ...
      prisma.signature.count({ where: { userId: actorId } }),
    ]);
