...
const createList = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // aligned web<->desktop.
        if (body.id) {
            const existing = yield prisma_1.prisma.supplyBatch.findUnique({
                where: { id: body.id },
            });
            if (existing)
                return res.code(200).send({ message: "Ok", data: existing });
        }
        const created = yield prisma_1.prisma.supplyBatch.create({
            data: Object.assign(Object.assign({}, (body.id ? { id: body.id } : {})), { title: body.title, inventoryBoxId: body.inventoryBoxId }),
        });
