...
exports.importUnitSupplyRsiExcel = exports.importUserSupplyRsiExcel = exports.exportSupplyExcel = exports.dataSetSupplies = exports.itemExcelFile = void 0;
...
const itemExcelFile = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if (!req.isMultipart()) {
...
exports.itemExcelFile = itemExcelFile;
const dataSetSupplies = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const rows = response.map((item, i) => {
            ...
            return ({
                no: i + 1,
                item: (_a = item.item) !== null && _a !== void 0 ? _a : "N/A",
                ref: item.code,
                date: (0, date_1.formatDate)(item.createdAt.toISOString()),
            });
        });
        worksheet.addRows(rows);
        // Set proper headers
        res.header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        res.header("Content-Disposition", "attachment; filename=SupporterReport.xlsx");
...
//   }
// };
const exportSupplyExcel = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const [file, data] = yield prisma_1.prisma.$transaction([
            prisma_1.prisma.supplytExcelExport.findUnique({
                where: {
                    id: params.id,
                },
            }),
            prisma_1.prisma.supplies.findMany({
                where: {
        ...
        const groupedData = data.reduce((groups, item) => {
            ...
            const processedItem = {
                ...
                desc: ((_a = item.SupplieRecieveHistory[0].supplyOrder) === null || _a === void 0 ? void 0 : _a.desc) || "N/A",
                dataSet: (_b = item.SuppliesDataSet) === null || _b === void 0 ? void 0 : _b.title,
            };
            // Group by suppliesDataSetId
            const groupKey = item.suppliesDataSetId || "ungrouped";
            if (!groups[groupKey]) {
                groups[groupKey] = {
                    dataSetTitle: ((_c = item.SuppliesDataSet) === null || _c === void 0 ? void 0 : _c.title) || "Ungrouped",
        ...
        const sampleTemplateUrl = "https://res.cloudinary.com/drhkb0ubf/raw/upload/v1765101238/SupplyReportTemplate_yoqkas.xlsx";
        const response = yield fetch(sampleTemplateUrl);
        ...
        // Send back to user
        res.header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        res.header("Content-Disposition", `attachment; filename="SupplyReport_${(file === null || file === void 0 ? void 0 : file.file_name) || "export"}.xlsx"`);
...
exports.exportSupplyExcel = exportSupplyExcel;
const importUserSupplyRsiExcel = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const params = req.body;
    if (!params.id)
        throw new errors_1.ValidationError("INVALID REQUIRED ID");
    if (!params.itemIds || params.itemIds.length === 0)
        throw new errors_1.ValidationError("INVALID REQUIRED ITEM IDS");
    try {
        const [user, items] = yield prisma_1.prisma.$transaction([
            ...
            prisma_1.prisma.supplyDispenseRecord.findMany({
                where: {
                    id: {
                        in: params.itemIds,
                    },
        ...
        if (items.length === 0)
            throw new errors_1.NotFoundError("ITEMS NOT FOUND");
        // Use the NEW .xlsx URL
        const sampleTemplateUrl = "https://res.cloudinary.com/drhkb0ubf/raw/upload/v1766728188/RIS_2_ombjjk_1_zovq3y.xlsx";
        const response = yield fetch(sampleTemplateUrl);
        if (!response.ok) {
            throw new errors_1.AppError("FAILED_TO_FETCH_TEMPLATE", 500, `Failed to fetch Excel template: ${response.status} ${response.statusText}`);
        }
        const arrayBuffer = yield response.arrayBuffer();
        const buffer = Buffer.from(arrayBuffer);
        const { Readable } = require("stream");
        ...
        if (!worksheet) {
            throw new errors_1.AppError("INVALID_TEMPLATE", 500, "Worksheet not found");
        }
        console.log(`Template loaded: ${worksheet.name}, Rows: ${worksheet.rowCount}`);
        ...
        worksheet.getRow(6).getCell("A").value = "Recipient:";
        worksheet.getRow(5).getCell("B").value = `${((_a = user.department) === null || _a === void 0 ? void 0 : _a.name) || "N/A"}`;
        items.forEach((item, index) => {
            ...
            row.getCell("A").value = index + 1; // Serial number
            row.getCell("B").value = ((_a = item.supply) === null || _a === void 0 ? void 0 : _a.quality) || "N/A";
            // Column C: Item description
            const cellC = row.getCell("C");
            cellC.value = `${((_b = item.supplyItem) === null || _b === void 0 ? void 0 : _b.item) || "N/A"} - ${item.desc || "N/A"}`;
        ...
        // Send the file
        res.header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        res.header("Content-Disposition", `attachment; filename="RIS_2_${user.firstName}_${user.lastName}.xlsx"`);
        return res.send(excelBuffer);
    }
    catch (error) {
        ...
        throw new errors_1.AppError("UNKNOWN_ERROR", 500, error instanceof Error ? error.message : "An unknown error occurred");
    }
});
exports.importUserSupplyRsiExcel = importUserSupplyRsiExcel;
const importUnitSupplyRsiExcel = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const params = req.body;
    if (!params.id)
        throw new errors_1.ValidationError("INVALID REQUIRED ID");
    if (!params.itemIds || params.itemIds.length === 0)
        throw new errors_1.ValidationError("INVALID REQUIRED ITEM IDS");
    try {
        const [unit, items] = yield prisma_1.prisma.$transaction([
            prisma_1.prisma.department.findUnique({
                where: {
                    id: params.id,
                },
                select: {
        ...
        // Send the file
        res.header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        res.header("Content-Disposition", `attachment; filename="RIS_2_${unit.name}.xlsx"`);
        return res.send(excelBuffer);
    }
    catch (error) {
        if (error instanceof prisma_1.Prisma.PrismaClientKnownRequestError) {
...
exports.importUnitSupplyRsiExcel = importUnitSupplyRsiExcel;
