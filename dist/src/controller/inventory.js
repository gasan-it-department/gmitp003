...
const createInventory = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (body.id) {
            const existing = yield prisma_1.prisma.inventoryBox.findUnique({
                where: { id: body.id },
            });
            if (existing)
                return res.code(200).send({ message: "OK", data: existing });
        }
        const check = yield prisma_1.prisma.inventoryBox.findFirst({
            where: {
                lineId: body.lineId,
        ...
        const response = yield prisma_1.prisma.inventoryBox.create({
            data: Object.assign(Object.assign({}, (body.id ? { id: body.id } : {})), { name: body.name, code: code, lineId: body.lineId, userId: body.userId, departmentId: body.departmentId || null, createdAt: new Date() }),
        });
        if (!response)
