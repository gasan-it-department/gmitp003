...
exports.barcodeLookupCandidates = exports.readScannedCode = exports.MAX_SCAN_CODE_LENGTH = exports.canonicalGtin = void 0;
 ...
 */
const canonicalGtin = (digits) => {
    let d = digits;
    while (d.length > 13 && d.startsWith("0"))
        d = d.slice(1);
    if (d.length === 12)
        d = "0" + d; // UPC-A → EAN-13 form
    return d;
};
exports.canonicalGtin = canonicalGtin;
/** Parse GS1 element strings: (01)…(17)…(10)… or their bare equivalent. */
const parseGs1Elements = (s) => {
    if (!/\(\d{2,4}\)/.test(s))
        return null;
    const out = { code: "", kind: "gtin" };
    const re = /\((\d{2,4})\)([^(]*)/g;
    let m;
    while ((m = re.exec(s))) {
        const ai = m[1];
        ...
        if (ai === "01" && /^\d{8,14}$/.test(val))
            out.code = (0, exports.canonicalGtin)(val);
        else if (ai === "17")
            out.expiry = gs1Date(val);
        else if (ai === "10" && val)
            out.lot = val;
    }
...
exports.MAX_SCAN_CODE_LENGTH = 64;
/**
 ...
 */
const readScannedCode = (raw) => {
    var _a;
    // Strip control characters (embedded newlines from multi-line QR would
    // break the keyboard-wedge path and the stored value).
    const s = (raw !== null && raw !== void 0 ? raw : "")
        .replace(new RegExp("[\u0000-\u001f\u007f]", "g"), " ")
        .replace(/\s+/g, " ")
        .trim();
    if (!s)
        return { code: "", kind: "plain", rejected: "Empty code." };
    for (const f of FOREIGN_PATTERNS) {
        if (f.re.test(s))
            return { code: "", kind: "plain", rejected: f.why };
    }
    const gs1 = (_a = parseDigitalLink(s)) !== null && _a !== void 0 ? _a : parseGs1Elements(s);
    if (gs1)
        return gs1;
    // A bare numeric barcode (EAN/UPC/GTIN) — normalize to one form.
    if (/^\d{8,14}$/.test(s))
        return { code: (0, exports.canonicalGtin)(s), kind: "gtin" };
    ...
    if (/^https?:\/\//i.test(s)) {
        return {
            code: "",
            kind: "plain",
            rejected: "That QR is a web link, not a medicine code. Scan the product's " +
                "barcode or its GS1 QR.",
        };
    }
    if (s.length > exports.MAX_SCAN_CODE_LENGTH) {
        return {
            code: "",
            kind: "plain",
...
exports.readScannedCode = readScannedCode;
/**
 ...
 */
const barcodeLookupCandidates = (raw) => {
    const reading = (0, exports.readScannedCode)(raw);
    const out = new Set();
    if (reading.code) {
...
exports.barcodeLookupCandidates = barcodeLookupCandidates;
